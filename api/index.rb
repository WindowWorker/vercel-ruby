...
Handler = Proc.new do |req, res|
  begin
    ...
    if (req_request_uri.include?('favicon') || req_request_uri.include?('apple-touch-icon')) && ((ref && ref.include?('rua.'))||(req['workerhost'] && req['workerhost'].include?('rua.')))
      res.header['location']='https://api.typescripts.org/corsFetch/https://archives.bulbagarden.net/media/upload/4/43/Spr_2s_112.png'
      res.status='301'
      next
    end
...
TEXT
    ...
    if (response.header['content-type']&&(!(response.header['content-type'].include?('text')))&&(!(response.header['content-type'].include?('script')))&&(!(req_request_uri.include?('.js'))))
      begin
        text=body
        text.encode('UTF-8')
